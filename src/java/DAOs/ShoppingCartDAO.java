...
public class ShoppingCartDAO {
    ...
    public int getQuantity(String cus_us) throws Exception {
        ...
        try {
            conn = DBcontext.DBConnection.connect();
            PreparedStatement ps = conn.prepareStatement("SELECT COUNT(name_food) as count FROM Cart WHERE userCus=?");
            ps.setString(1, cus_us);
    ...
    public int getTotalPrice(int idFood) throws Exception {
        int count = 0;
        try {
            conn = DBcontext.DBConnection.connect();
            PreparedStatement ps = conn.prepareStatement("SELECT price, quantity FROM Cart WHERE idFood=?");     
            ps.setInt(1, idFood);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public int getQuantityOrder(String cus_us) throws Exception {
        ...
        try {
            conn = DBcontext.DBConnection.connect();
            PreparedStatement ps = conn.prepareStatement("SELECT sum(quantity) as count FROM Cart WHERE userCus=?");
            ps.setString(1, cus_us);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                count = rs.getInt("count");
